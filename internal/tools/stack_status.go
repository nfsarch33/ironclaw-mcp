...
// StackStatusHandler handles the helixon_stack_status MCP tool.
...
// routerURL is read from HELIXON_ROUTER_URL env var, defaulting to http://127.0.0.1:8080.
func NewStackStatusHandler(client IronclawClient) *StackStatusHandler {
	routerURL := os.Getenv("HELIXON_ROUTER_URL")
	if routerURL == "" {
...
// Tool returns the helixon_stack_status MCP tool definition.
func (h *StackStatusHandler) Tool() mcp.Tool {
	return mcp.NewTool(
		"helixon_stack_status",
		mcp.WithDescription("Get combined health status of the Helixon stack: LLM router nodes, GPU availability, gateway status, queue depth, and inflight requests."),
	)
}

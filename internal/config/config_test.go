...
func TestLoad_Defaults(t *testing.T) {
	...
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "")
...
func TestLoad_CustomValues(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "http://myhost:4000")
	t.Setenv("HELIXON_ALLOW_NON_LOCALHOST", "true")
	t.Setenv("HELIXON_API_KEY", "secret123")
	t.Setenv("MCP_TRANSPORT", "sse")
	t.Setenv("MCP_SSE_ADDR", ":9090")
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "60")
...
func TestLoad_InvalidTimeout(t *testing.T) {
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "not-a-number")
	_, err := Load()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "HELIXON_TIMEOUT_SECONDS")
}

func TestLoad_InvalidTransport(t *testing.T) {
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "30")
...
func TestLoad_EmptyBaseURL(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "cannot-be-set-empty-via-setenv")
	...
	err := c.validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "HELIXON_BASE_URL")
}

func TestLoad_InvalidBaseURL_NonLoopback(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "http://evil.example.com:3000")
	t.Setenv("HELIXON_ALLOW_NON_LOCALHOST", "")
	_, err := Load()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "HELIXON_BASE_URL")
	assert.Contains(t, err.Error(), "HELIXON_ALLOW_NON_LOCALHOST")
}

func TestLoad_AllowNonLocalhost(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "http://my-helixon.local:3000")
	t.Setenv("HELIXON_ALLOW_NON_LOCALHOST", "true")
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "30")
	t.Setenv("MCP_TRANSPORT", "stdio")
	cfg, err := Load()
	require.NoError(t, err)
	assert.Equal(t, "http://my-helixon.local:3000", cfg.IronclawBaseURL)
	assert.True(t, cfg.AllowNonLocalhost)
}

func TestLoad_InvalidBaseURL_Scheme(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "ftp://localhost:3000")
	t.Setenv("HELIXON_TIMEOUT_SECONDS", "30")
	_, err := Load()
	require.Error(t, err)
...
func TestLoad_TimeoutBounds(t *testing.T) {
	t.Setenv("HELIXON_BASE_URL", "http://localhost:3000")
	t.Setenv("MCP_TRANSPORT", "stdio")

	t.Run("too_low", func(t *testing.T) {
		t.Setenv("HELIXON_TIMEOUT_SECONDS", "0")
		_, err := Load()
		require.Error(t, err)
	...
	t.Run("too_high", func(t *testing.T) {
		t.Setenv("HELIXON_TIMEOUT_SECONDS", "300")
		_, err := Load()
		require.Error(t, err)

...
func TestMCCLIPath_Default(t *testing.T) {
	t.Setenv("MCCLI_PATH", "")
	assert.Equal(t, "mc-cli", MCCLIPath())
}

func TestMCCLIPath_Override(t *testing.T) {
	t.Setenv("MCCLI_PATH", "/custom/bin/mc-cli")
	assert.Equal(t, "/custom/bin/mc-cli", MCCLIPath())
}

func TestLegacyMCCLIToolsEnabled(t *testing.T) {
	cases := []struct {
		val  string
		want bool
	}{
		// Off by default — generic-by-default posture.
		{"", false},
		{"0", false},
		{"false", false},
		{"no", false},
		{"off", false},
		{"random", false},
		// Truthy variants enable the legacy mc-cli surface.
		{"1", true},
		{"true", true},
		{"TRUE", true},
		{"yes", true},
		{"YES", true},
		{"on", true},
		{"  1  ", true}, // whitespace is trimmed
	}
	for _, c := range cases {
		t.Run(c.val, func(t *testing.T) {
			t.Setenv("IRONCLAW_MCP_LEGACY_TOOLS", c.val)
			assert.Equal(t, c.want, LegacyMCCLIToolsEnabled())
		})
	}
}
